...
host.BrowserHost = class {
  ...
  async start() {
    if (this._meta.file) {
      ...
      if (this._view.accept(url)) {
        const identifier =
          Array.isArray(this._meta.identifier) &&
          this._meta.identifier.length === 1
            ? this._meta.identifier[0]
            : null;
        const name = this._meta.name || null;
        const status = await this._openModel(
          this._url(url),
          identifier || null,
          name,
        );
    ...
    const params = new Map(
      search ? new URLSearchParams(this.window.location.search) : [],
    );
    ...
    if (url) {
      ...
      if (
        this._view.accept(identifier || location) &&
        location.indexOf("*") === -1
      ) {
    ...
    let dirPicker = getInstance("home_cloud_dir_picker");
    let filePicker = getInstance("home_cloud_file_picker");

    filePicker.addEventListener("submit", async (raw) => {
      const selected = dataTransfer(raw);
      invoke(filePicker, "close");

      const resp = await fetch(
        `https://${window.location.host}/_lzc/files/home/${selected.basename}`,
      );
      const b = await resp.blob();
      const file = new File([b], "example.txt", { type: b.type });
      console.log("file:", file);
      this._open(file, [file]);
    });

    filePicker.addEventListener("visible", function () {
      invoke(this, "close");
    });
    ...
    if (openFileButton && openFileDialog) {
      openFileButton.addEventListener("click", () => {
        invoke(filePicker, "init");
        invoke(filePicker, "open");
        return;
        this.execute("open");
      });
      const mobileSafari =
      ...
      request.onload = () => {
        ...
        if (request.status === 200) {
          ...
          const error = new Error(
            `The web request failed with status code '${request.status}'.`,
          );
          error.context = url;
          reject(error);
        }
  ...
  async _openModel(url, identifier, name) {
    ...
    try {
      ...
      context = new host.BrowserHost.Context(
        this,
        url,
        identifier,
        name,
  ...
  async _openGist(gist) {
    ...
    try {
      ...
      let file = null;
      if (!message) {
        file = Object.values(json.files).find((file) =>
          this._view.accept(file.filename),
        );
        if (!file) {
        ...
        switch (error.code) {
          ...
          default:
            message = error.message
              ? error.message
              : `File read '${error.code}' error '${file}'.`;
            break;
        }
...
host.BrowserHost.FileStream = class {
  ...
  _fill(length) {
    if (this._position + length > this._length) {
      throw new Error(
        `Expected ${this._position + length - this._length} more bytes. The file might be corrupted. Unexpected end of file.`,
      );
    }
    if (
      !this._buffer ||
      ...
      const length = Math.min(
        0x10000000,
        this._start + this._length - this._offset,
      );
